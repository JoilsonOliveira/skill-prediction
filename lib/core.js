...
module.exports = function SkillPrediction(dispatch) {
    ...
    function handleStartSkill(type, event, info, data, send) {
        ...
        if (mounted && config.mountCheck) {
            ...
            if(config.FakeSmt) sendSystemMessage('SMT_PROHIBITED_ACTION_ON_RIDE')
            return false
        }
        ...
        if (!info.noRetry)
            retry(() => {
                if ((config.skillRetryAlways && type != 'C_PRESS_SKILL') || currentAction && currentAction.skill == skill && !serverConfirmedAction) return toServerLocked(data);
                return false
            })
    }
