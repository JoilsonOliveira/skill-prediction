...
const {protocol, sysmsg} = require('tera-data-parser'),
    ...
    silence = require('../config/data/basicCC').reduce((map, value) => { // Convert array to object for fast lookup
        map[value] = true;
        return map
    }, {});
    evasivecc = require('../config/data/evasiveCC').reduce((map, value) => { // Convert array to object for fast lookup
        map[value] = true;
        return map
    }, {});
    extcc = require('../config/data/extendedCC').reduce((map, value) => { // Convert array to object for fast lookup
        map[value] = true;
        return map
    }, {});
...
module.exports = function SkillPrediction(dispatch) {
    ...
    command.add('sp', (option, value) => {
        switch (option) {
            case 'info':
                command.message('Unofficial SP. Date:13/12/17');
            ...
            case 'timeout':
                ...
                if (inCombat || sending) {
                    ...
                    command.message(`[Skill Prediction] New server timeout ${config.server_timeout}`);
                }
                break;
            case 'debug':
                if (config.debug)
...
// This is not correct, but the midpoint location seems to be "close enough" for the client to not teleport the player
    ...
    function handleStartSkill(type, event, info, data, send) {
        ...
        if (abnormality.inMap(silence)) {
            sendCannotStartSkill(event.skill);
            return false
        }

        if(info.evasive && abnormality.inMap(evasivecc)) {
            sendCannotStartSkill(event.skill);
            return false
        }

        if(info.extended && abnormality.inMap(extcc)) {
            sendCannotStartSkill(event.skill);
            return false
        }
