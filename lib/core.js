...
const {protocol, sysmsg} = require('tera-data-parser'),
    ...
    silence = require('../config/data/silenceCC').reduce((map, value) => { // Convert array to object for fast lookup
        map[value] = true;
        return map
    }, {});
    evasive = require('../config/data/movementCC').reduce((map, value) => { // Convert array to object for fast lookup
        map[value] = true;
        return map
    }, {});
...
// This is not correct, but the midpoint location seems to be "close enough" for the client to not teleport the player
    ...
    function handleStartSkill(type, event, info, data, send) {
        ...
        if(info.evasive && abnormality.inMap(evasive)) {
            sendCannotStartSkill(event.skill);
            return false
        }
